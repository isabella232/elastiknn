...
package object api {
  ...
  object NearestNeighborsQuery {
    final case class Exact(field: String, similarity: Similarity, min_score:Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends NearestNeighborsQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
    }

    final case class SparseIndexed(field: String, similarity: Similarity, min_score:Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends NearestNeighborsQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
    }
    ...
    final case class JaccardLsh(field: String, candidates: Int, min_score: Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends ApproximateQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
    ...
    final case class HammingLsh(field: String, candidates: Int, min_score: Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends ApproximateQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
      override def withCandidates(candidates: Int): ApproximateQuery = copy(candidates = candidates)
      override def similarity: Similarity = Similarity.Hamming
    }

    final case class CosineLsh(field: String, candidates: Int, min_score: Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends ApproximateQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
      override def withCandidates(candidates: Int): ApproximateQuery = copy(candidates = candidates)
      override def similarity: Similarity = Similarity.Cosine
    }

    final case class L2Lsh(field: String, candidates: Int, probes: Int = 0, min_score: Double = Double.NegativeInfinity, vec: Vec = Vec.Empty()) extends ApproximateQuery {
      override def withVec(v: Vec): NearestNeighborsQuery = copy(vec = v)
      override def withCandidates(candidates: Int): ApproximateQuery = copy(candidates = candidates)
      override def similarity: Similarity = Similarity.L2
    }

    final case class PermutationLsh(field: String, similarity: Similarity, candidates: Int, min_score: Double = Double.NegativeInfinity, vec: Vec = Vec.Empty())
